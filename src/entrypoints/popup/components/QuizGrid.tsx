...
import { createProgress, promptQuizDownload } from '@/utilities/quizzes'
...
export function QuizGrid (): JSX.Element {
  ...
  const handleQuizPlay = (quiz: Quiz) => () => {
    ...
    browser.sidebarAction.open().catch((error: Error) => {
...

    {
      ...
      getActions: ({ id, row: quiz }: GridRowParams<Quiz>) => {
        ...
        if (isInEditMode) {
          return [
            <GridActionsCellItem
              sx={cellItemHoverColor(Palette.Donatello)}
              key='save'
              label={t('save')}
              title={t('save')}
              icon={<Save />}
            ...
            />,
            <GridActionsCellItem
              sx={cellItemHoverColor(Palette.Raphael)}
              key='cancel'
              label={t('cancel')}
              title={t('cancel')}
              icon={<Cancel />}
        ...
        return [
          <GridActionsCellItem
            sx={cellItemHoverColor(Palette.Michelangelo)}
            key='edit'
            label={t('edit')}
            title={t('edit')}
            icon={<Edit />}
          ...
          />,
          <GridActionsCellItem
            sx={cellItemHoverColor(Palette.Leonardo)}
            key='play'
            label={t('play')}
            title={t('play')}
            icon={<PlayCircle />}
